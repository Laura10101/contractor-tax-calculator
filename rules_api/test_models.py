...
def create_mock_jurisdiction():
    ...
    return Jurisdiction.objects.create(
        name='Test Jurisdiction ' + str(jurisdiction_count)
    )
...
def create_mock_variable_table(
    salary=9000,
    dividends=50000,
...
def create_mock_ruleset():
    ...
    tax_category = TaxCategory.objects.create(
        name='Test Category ' + str(TaxCategory.objects.count())
    )
...
def create_mock_simple_tiered_rate_rule(
    min_value,
    max_value,
    variable_name,
    tax_rate
...
def create_mock_secondary_tiered_rate_rule(
    primary_rule,
    variable_name,
    ruleset=None
):
...
def create_mock_secondary_rule_tier(secondary_rule, primary_tier, tier_rate):
    return SecondaryRuleTier.objects.create(
...
def create_mock_simple_secondary_tiered_rate_rule(
    min_value,
    max_value,
    primary_variable,
    secondary_variable,
    ...
    secondary_tier = create_mock_secondary_rule_tier(
        secondary_rule,
        primary_tier,
        secondary_rate
    )
...
def test_flat_rate_calculate():
    ...
    expected_tax = variables['company_profit'] * (flat_rate / 100)
    assert results.results.values()[0]['tax_payable'] == expected_tax
...
def test_rule_tier_calculate_where_income_on_lower_boundary():
    ...
    assert len(results.results.values()) == 1
    assert results.results.values()[0] is not None
    expected_tax = round((variables['salary'] - 9000) * (20 / 100), 2)
    assert results.results.values()[0]['tax_payable'] == expected_tax
...
def test_rule_tier_calculate_where_income_within_boundaries():
    ...
    assert len(results.results.values()) == 1
    assert results.results.values()[0] is not None
    expected_tax = round((25000 - 9000) * (20 / 100), 2)
    assert results.results.values()[0]['tax_payable'] == expected_tax
...
def test_rule_tier_calculate_where_income_on_upper_boundary():
    ...
    assert len(results.results.values()) == 1
    assert results.results.values()[0] is not None
    expected_tax = round((45000 - 9000) * (20 / 100), 2)
    assert results.results.values()[0]['tax_payable'] == expected_tax
...
@pytest.mark.django_db
def test_rule_tier_calc_where_no_upper_bound_and_income_above_lower_bound():
...
@pytest.mark.django_db
def test_secondary_tier_calc_where_total_income_below_lower_bound():
    rule = create_mock_simple_secondary_tiered_rate_rule(
        1000000, 4000000, 'salary', 'dividends', 20, 8
    )
    variables = create_mock_variable_table()
    results = create_mock_ruleset_calculation_result()
    rule.calculate(variables, results)
...
@pytest.mark.django_db
def test_secondary_tier_where_primary_income_on_lower_and_no_second_income():
    primary_income = 10000
    secondary_income = 0
    tier_min = primary_income
    tier_max = 45000
    rule = create_mock_simple_secondary_tiered_rate_rule(
        tier_min, tier_max, 'salary', 'dividends', 20, 8
    )
    variables = create_mock_variable_table(
        salary=primary_income,
        dividends=secondary_income
    )
    results = create_mock_ruleset_calculation_result()
    rule.calculate(variables, results)
    assert len(results.results.values()) == 0
...
@pytest.mark.django_db
def test_second_tier_where_primary_income_on_lower_and_total_within_bounds():
    primary_income = 10000
    ...
    assert len(results.results.values()) == 1
    assert results.results.values()[0] is not None
    expected_tax = round(secondary_income * (8 / 100), 2)
    assert results.results.values()[0]['tax_payable'] == expected_tax


@pytest.mark.django_db
def test_second_tier_where_primary_income_and_total_within_bounds():
...
@pytest.mark.django_db
def test_second_tier_where_primary_income_within_bounds_and_total_exceeds():
    ...
    assert len(results.results.values()) == 1
    assert results.results.values()[0] is not None
    expected_tax = round(tier_amount_remaining * (8 / 100), 2)
    assert results.results.values()[0]['tax_payable'] == expected_tax


@pytest.mark.django_db
def test_second_tier_where_primary_income_on_upper_bound_and_total_exceeds():
...
@pytest.mark.django_db
def test_second_tier_where_primary_income_above_upper_boundary():
...
def test_tiered_rule_iteration_with_multiple_tiers_defined():
    ...
    expected_tax = round((75000 - 45001) * (45 / 100), 2)
    assert results.results.values()[1]['tax_payable'] == expected_tax
...
def test_secondary_tiered_rule_iteration_with_single_tier_defined():
    secondary_rate_rule = create_mock_simple_secondary_tiered_rate_rule(
        10000,
        45000,
        'salary',
        'dividends',
...
def test_secondary_tiered_rule_iteration_with_multiple_tiers_defined():
    ...
    second_primary_tier = create_mock_rule_tier(
        secondary_rate_rule.primary_rule,
        45001,
        100000,
        45
    )
    second_secondary_tier = create_mock_secondary_rule_tier(
        secondary_rate_rule,
        second_primary_tier,
        40
    )
    ...
    # Similarly none of the second secondary tier has been used by
    # primary income. However, dividends is not enough to use all of
    # this up
    tier_remaining = (45001 - variables['salary'])
    dividends_remaining = variables['dividends'] - tier_remaining
...
def test_ruleset_iteration_with_single_rule_defined():
    ...
    expected_tax = round(variables['dividends'] * (8 / 100), 2)
    assert ruleset_result.results.values()[0]['tax_payable'] == expected_tax
